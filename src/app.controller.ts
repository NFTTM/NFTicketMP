...
export class AppController {
  ...
  @Post('ipfs-save')
  @ApiOperation({
    summary: 'Save file (image) to IPFS',
    description: 'Read a registered file from the local json-db and push it to IPFS',
  })
  @ApiResponse({
    status: 200,
    description: 'File push to IPFS',
  })
  @ApiResponse({
    status: 503,
    description: 'Server Error',
  ...
  @Get('nfts/:testnet/:contractAddress')
  @ApiOperation({
  ...
  async getAllNFTs(@Param('testnet') testnet: string,
    @Param('contractAddress') contractAddress: string) {
    try {
      const result = this.appService.getAllNFTs(testnet, contractAddress);
      return result;
    } catch (error) {
      throw new HttpException(error.message, 503);
    }
  ...
  @Get('nfts/:testnet/:contractAddress/:id')
  @ApiOperation({
  ...
  async getNFTByID(@Param('testnet') testnet: string, @Param('contractAddress') contractAddress: string, @Param('id') id: number) {
    try {
      const result = this.appService.getNFTByID(testnet, contractAddress, id);
      return result;
    } catch (error) {
      throw new HttpException(error.message, 503);
    }
